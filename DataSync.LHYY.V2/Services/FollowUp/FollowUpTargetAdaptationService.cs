...
public sealed class FollowUpTargetAdaptationService(IConfiguration configuration)
{
    ...
    internal static string AdaptRow(string targetSchema, string targetTable, string row)
    {
        ...
        if (!IsEligiblePatientEvent(patientEvent))
        {
            patientEvent["form_set_id"] = null;
            patientEvent["form_set_name"] = null;
            patientEvent["event_type_definition_id"] = null;
        }
    ...
    private static bool IsEligiblePatientEvent(JsonObject row)
    {
        if (!row.TryGetPropertyValue("is_valid", out var isValidNode)
            || isValidNode is not JsonValue isValidValue
            || !isValidValue.TryGetValue<bool>(out var isValid)
            || !isValid)
            return false;
        if (!row.TryGetPropertyValue("form_set_id", out var formSetId)
            || formSetId is null
            || string.IsNullOrWhiteSpace(formSetId.ToString()))
            return false;

        var eventType = StringValue(row, "event_type");
        var status = StringValue(row, "event_status");
        if (string.IsNullOrEmpty(status))
            status = StringValue(row, "status");

        if (status is "已审核" or "已随访")
            return true;
        if (eventType == "转诊记录" && status == "已确认")
            return true;

        var hasInputTime = row.TryGetPropertyValue("input_time", out var inputTime)
                           && inputTime is not null
                           && !string.IsNullOrWhiteSpace(inputTime.ToString());
        return eventType is "预问诊" or "门诊签到"
               && hasInputTime
               && status is "门诊结束" or "办理住院" or "入组随访" or "转诊";
    }

    private static string StringValue(JsonObject row, string propertyName) =>
        row.TryGetPropertyValue(propertyName, out var value) && value is JsonValue
            ? value.ToString()
            : string.Empty;
}
